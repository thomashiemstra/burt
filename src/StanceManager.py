...
class StanceManager(object):
    ...
    def __init__(self):
        self.configs = {
            ...
            Stance.FAST: StanceConfig(max_x_velocity=0.5, max_y_velocity=0.3, max_yaw_rate=0.7, swing_time=0.09,
                                        overlap_time=0.09, delay_factor=1.2, alpha=0.5, beta=0.5, z_clearance=0.074,
                                        delta_x=0.1, x_shift=0.02, delta_y=0.012),
        }
...
class Stance(Enum):
    ...
    def next(self):
        val = np.clip(self.value + 1, 1, len(Stance))
        return Stance(val)
